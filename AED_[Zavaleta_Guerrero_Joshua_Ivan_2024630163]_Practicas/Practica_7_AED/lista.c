...
int cambiarElementoPosicion(Lista l, Elemento e, int posicion)
{
	Nodo aux=NULL, contiguo=NULL, sig= NULL;
	int k=0;
...
//i1 -> intervalo 1, i2 -> intervalo 2
int generarListaAleatoria(Lista l,int n,char i1, char i2)
{
	if (l==NULL)
	  return(FALSO);
	...
	{
		crearElemento(&auxe);
		escribirDato(auxe,(rand()%res)+i1);
		insertarElementoPosicionVariante(l,auxe,l->longitud);
		imprimirLista(l);
	}
}
...
int vaciarLista(Lista l)
{
	...
	while(l->longitud != 0)
	{
		quitarElementoPosicion(l,&(l->ultimo->elem),l->longitud-1);
		imprimirLista(l);
	}
